...
class TestShellInterpreter:
    ...
    def test_working_directory(self):
        ...
        # Check initial directory (could be / or /home/user depending on initialization)
        shell.execute("pwd").strip()
        # Change to root for consistent testing
        shell.execute("cd /")
        result = shell.execute("pwd")
        assert result.strip() == "/"
        ...
        # Home directory
        home_dir = shell.environ["HOME"]
        # Create home directory if it doesn't exist
        shell.execute(f"mkdir -p {home_dir}")
        shell.execute("cd ~")
        result = shell.execute("pwd")
        assert home_dir in result or result.strip() == home_dir
