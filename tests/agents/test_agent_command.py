...
"""
    ...
    def test_missing_agent_file(self):
        ...
        result = self.command.run("nonexistent.agent")
        assert "No such file" in result

    def test_load_agent_file(self):
        """Test loading an agent file"""
        with patch.object(
            self.shell.agent_manager, "run_process", new_callable=AsyncMock
        ) as mock_run:
            mock_run.return_value = "Agent response"

            result = self.command.run("/test.agent")
    ...
    def test_background_execution(self):
        ...
        with patch.object(
            ...
            result = self.command.run("/test.agent -b")
    ...
    def test_list_agents(self):
        ...
        with patch.object(
            self.shell.agent_manager,
            "list_processes",
            return_value=[mock_proc1, mock_proc2],
        ):
            result = self.command.run("-l")
    ...
    def test_show_status(self):
        ...
        mock_proc.output_buffer = (
            "Test output that is very long and should be truncated"
        )
        mock_proc.error_buffer = ""

        with patch.object(
            self.shell.agent_manager, "get_process", return_value=mock_proc
        ):
            result = self.command.run("-s agent_1")
    ...
    def test_show_status_not_found(self):
        ...
        with patch.object(self.shell.agent_manager, "get_process", return_value=None):
            result = self.command.run("-s nonexistent")
            assert "No such agent process" in result
    ...
    def test_input_file(self):
        ...
        with patch.object(
            ...
            result = self.command.run("/test.agent -i /input.txt")
    ...
    def test_output_file(self):
        ...
        with patch.object(
            ...
            result = self.command.run("/test.agent -o /output.txt")
    ...
    def test_timeout_option(self):
        ...
        with patch.object(self.shell.agent_manager, "create_process") as mock_create:
            ...
            with patch.object(
                ...
                result = self.command.run("/test.agent -t 30")
    ...
    def test_invalid_arguments(self):
        ...
        result = self.command.run("--invalid-flag")
