...
def test_awk_calculations(shell):
    ...
    # Average calculation
    result = shell.execute('awk \'{sum+=$1; count++} END {print sum/count}\' /tmp/numbers.txt')
    assert "20" in result
...
def test_awk_begin_end(shell):
    ...
    result = shell.execute('awk \'BEGIN {print "Start"} {sum+=$1} END {print "Total:", sum}\' /tmp/data.txt')
...
def test_awk_empty_file(shell):
    ...
    result = shell.execute('awk \'BEGIN {print "start"} END {print "end"}\' /tmp/empty.txt')
    assert "start" in result
...
def test_nonexistent_file_handling(shell):
    ...
    assert "No such file" in shell.execute('grep "test" /tmp/nonexistent.txt') or \
           shell.execute('grep "test" /tmp/nonexistent.txt') == ""
    
    assert "No such file" in shell.execute('sed "s/a/b/" /tmp/nonexistent.txt') or \
           shell.execute('sed "s/a/b/" /tmp/nonexistent.txt') == ""
    
    assert "No such file" in shell.execute('awk "{print}" /tmp/nonexistent.txt') or \
           shell.execute('awk "{print}" /tmp/nonexistent.txt') == ""
