...
def test_client_config(signature_version, addressing_style="auto", use_ssl=True):
    ...
    try:
        ...
        try:
            ...
            results["list_objects_v1"] = (
                f"✓ ({len(response.get('Contents', []))} objects)"
            )
        except Exception as e:
            error_code = (
                e.response.get("Error", {}).get("Code", "")
        ...
        try:
            response = client.create_multipart_upload(
                Bucket=bucket_name, Key="_test_multipart.txt"
            )
            upload_id = response["UploadId"]
            client.abort_multipart_upload(
                Bucket=bucket_name, Key="_test_multipart.txt", UploadId=upload_id
            )
        ...
        try:
            ...
            results["get_bucket_location"] = (
                f"✓ ({response.get('LocationConstraint', 'us-east-1')})"
            )
        except Exception as e:
...
for sig_ver, addr_style, use_ssl in configs_to_test:
    if test_client_config(sig_ver, addr_style, use_ssl):
        ...
        config = Config(signature_version=sig_ver, s3={"addressing_style": addr_style})
        client = boto3.client(
            "s3",
            endpoint_url=endpoint_url,
            aws_access_key_id=access_key,
