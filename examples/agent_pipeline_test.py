...
Code Reviews: 18"""
    ...
    print("Command: agent /agents/analyzer.agent -i /data/report.txt -o /data/analysis.txt")
    result = shell.execute("agent /agents/analyzer.agent -i /data/report.txt -o /data/analysis.txt")
    ...
    for i in range(3):
        shell.fs.write_file(f"/data/input{i}.txt", f"Test data {i}\nProcessing item {i}")
    ...
    for i in range(3):
        shell.execute(f"agent /agents/analyzer.agent -i /data/dataset{i}.txt -o /data/result{i}.txt")
