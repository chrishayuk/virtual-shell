...
Line 3""",
    ...
    help_text = (
        shell.commands.get("agent").get_help()
        if "agent" in shell.commands
        else "Agent command not found"
    )
    ...
    print(
        "Command: agent /agents/assistant.agent -i /workspace/question.txt -o /workspace/answer.txt"
    )
