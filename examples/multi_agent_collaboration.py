...
4. Save tasks to a file"""
    ...
    result = shell.execute(
        "cat /project/requirements.txt | agent /agents/analyst.agent"
    )
    shell.fs.write_file("/project/communication/task_list.txt", result)
    print(
        f"   Output: {result[:150]}..." if len(result) > 150 else f"   Output: {result}"
    )
    ...
    result = shell.execute(
        "cat /project/communication/task_list.txt | agent /agents/architect.agent"
    )
    ...
    result = shell.execute(
        "agent /agents/developer.agent -i /project/communication/dev_input.txt"
    )
...
Please provide a final project summary."""
    ...
    result = shell.execute(
        "agent /agents/pm.agent -i /project/communication/status.txt"
    )
    shell.fs.write_file("/project/communication/final_summary.txt", result)
    print(
        f"   Output: {result[:200]}..." if len(result) > 200 else f"   Output: {result}"
    )
