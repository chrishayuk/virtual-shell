...
class AgentTask:
    ...
    def __init__(
        self, description: str, commands: List[str], verify_command: str = None
    ):
...
class CodingAgent:
    ...
    async def execute_task(self, task: AgentTask):
        ...
        if task.verify_command:
            ...
            async for chunk in self.manager.run_command(
                self.session_id, task.verify_command
            ):
    ...
    async def analyze_code(self, file_path: str):
        ...
        # Read the file
        async for chunk in self.manager.run_command(
            self.session_id, f"cat {file_path}"
        ):
            chunk.data.split("\n")
    ...
    async def generate_summary(self):
        ...
        if session:
            ...
            print("\n📁 Project Structure:")
            async for chunk in self.manager.run_command(
                self.session_id, "find . -type f -name '*.py' | head -10"
            ):
                if chunk.data.strip():
...
async def build_web_api_project():
    ...
    try:
        ...
        if session:
            ...
            print(f"  Working Directory: {state.cwd}")
            print(
                f"  Environment Variables Set: {len([k for k in state.env.keys() if k not in ['HOME', 'USER', 'PATH', 'PWD', 'OLDPWD']])}"
            )
...
async def simple_workflow_demo():
    ...
    try:
        ...
        if session:
            ...
            print("  Directories created: 3")

            # Show final structure
            print("\n  Final project structure:")
            async for chunk in manager.run_command(
                agent.session_id, "find . -type f | head -10"
            ):
                if chunk.data.strip():
                    for line in chunk.data.strip().split("\n"):
                        print(f"    {line}")
...
if __name__ == "__main__":
    ...
    print(
        "Showing how AI agents use persistent sessions for complex development tasks\n"
    )
