...
class AgentCommand(ShellCommand):
    ...
    # Agent command is async native - only implements execute_async
    # The base class will detect this and handle async/sync coordination through run()
    ...
    async def execute_async(self, args):
        ...
        parser.add_argument(
            "-l", "--list", action="store_true", help="List running agents"
        )
        ...
        parser.add_argument(
            "-t", "--timeout", type=int, metavar="SEC", help="Timeout in seconds"
        )
        ...
        try:
            ...
            if self.shell.fs.exists(agent_path):
                ...
                if self.shell.fs.exists(agent_path_with_ext):
                    definition = AgentDefinition.from_file(
                        agent_path_with_ext, self.shell.fs
                    )
                else:
                    return (
                        f"agent: cannot open '{parsed_args.agent_file}': No such file"
                    )
        ...
        process = self.shell.agent_manager.create_process(
            definition, background=parsed_args.background
        )

        if parsed_args.background:
            # Run in background
            task = asyncio.create_task(self._run_background_safe(process, input_data))
            process.task = task
