...
class DuCommand(ShellCommand):
    ...
    def execute(self, args: List[str]) -> str:
        ...
        parser.add_argument(
            "-k", action="store_true", help="Display sizes in kilobytes (default)"
        )
        parser.add_argument(
            "-m", action="store_true", help="Display sizes in megabytes"
        )
        parser.add_argument(
            "-b", "--bytes", action="store_true", help="Display sizes in bytes"
        )
        parser.add_argument(
            "-s",
            "--summarize",
        ...
        parser.add_argument(
            "--time-style", type=str, help="Time format style (full-iso, long-iso, iso)"
        )
        parser.add_argument(
            "--exclude",
            type=str,
        ...
        parser.add_argument(
            "-B", "--block-size", type=str, help="Scale sizes by SIZE before printing"
        )
        parser.add_argument(
            "-D",
            "--dereference-args",
        ...
        parser.add_argument(
            "--files0-from", type=str, help="Read null-terminated file names from FILE"
        )
        parser.add_argument(
            "--threshold", type=str, help="Exclude entries smaller/greater than SIZE"
        )
                    ...
                    for item_path, size in dir_items:
                        ...
                        if parsed_args.time:
                            ...
                            if parsed_args.time_style:
                                mod_time = self._format_time(
                                    mod_time, parsed_args.time_style
                                )
