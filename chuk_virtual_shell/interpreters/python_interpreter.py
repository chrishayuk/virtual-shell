...
class VirtualPythonInterpreter:
    ...
    def _create_namespace(self) -> Dict[str, Any]:
        ...
        virtual_os.path.exists = lambda path: shell_fs.exists(shell_fs.resolve_path(path))  # type: ignore
        virtual_os.path.isfile = lambda path: shell_fs.is_file(shell_fs.resolve_path(path))  # type: ignore
        virtual_os.path.isdir = lambda path: shell_fs.is_dir(shell_fs.resolve_path(path))  # type: ignore
    ...
    def run_script_sync(self, script_path: str, args: Optional[List[str]] = None) -> str:
