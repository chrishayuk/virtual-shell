...
class AgentDefinition:
    ...
    @classmethod
    def from_string(cls, content) -> "AgentDefinition":
        """Parse agent definition from a string or bytes"""
        ...
        # Check for agent shebang (optional for now)
        if lines[0].startswith("#!agent"):
            # Remove shebang and parse rest as YAML
            yaml_content = "\n".join(lines[1:])
        else:
            # No shebang, treat entire content as YAML
