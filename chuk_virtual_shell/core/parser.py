...
class CommandParser:
    ...
    def parse_redirection(cmd_line: str) -> Dict:
        ...
        """
        result = {"command": cmd_line, "input": None, "output": None, "append": False}
        ...
        if "<" in cmd_line:
            ...
            if not parser.is_quoted(cmd_line, pos):
                ...
                if result["input"] and isinstance(result["input"], str):
                    try:
        ...
        if not result["input"]:
            ...
            if (
                result["command"]
                and isinstance(result["command"], str)
                and ">>" in result["command"]
            ):
    ...
    def parse_pipeline_redirection(cmd_line: str) -> Dict:
        ...
        Returns:
            Dictionary with:
                ...
                - 'append': Boolean indicating append mode
        """
        result = {"pipeline": cmd_line, "input": None, "output": None, "append": False}
        ...
        if (
            result["pipeline"]
            and isinstance(result["pipeline"], str)
            and ">>" in result["pipeline"]
        ):
